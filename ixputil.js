(function (exports) {
  ...
  exports.ipack = ipack = function (fmt, data) {
    ...
    switch (fmt.substring(0, 1)) {
      ...
      case 's':
        ...
        if (packed.length < wide) {
          packed = pad(packed, wide, '\0');
        }
        return packed;
      case 'S':
        return counted(data, wide);
      default:
        throw 'pack: unknown type is ' + fmt + '.';
    }
  };
  ...
  var iunpack = function (fmt, data) {
    ...
    switch (fmt.substring(0, 1)) {
      case 'i':
        return asInteger(data.substring(0, wide));
      case 'b':
      case 's':
        return data.substring(0, wide);
      case 'S':
        //get actual count
      ...
      case 'R':
        ...
        while (data.length > 0) {
          ...
          if (len < 1) {
            throw 'invalid size in array: ' + len + ' <<' + data + '>>';
          }
        ...
        return out;
      default:
        throw 'unpack: unknown type is ' + fmt;
    }
  };
