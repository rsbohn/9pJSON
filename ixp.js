...
(function (exports) {
  ...
  var packets = {
    100: {
      name: 'Tversion',
      fmt: ['i4:size', 'i1:type', 'i2:tag', 'i4:msize', 'S2:version'],
    },
    101: {
      name: 'Rversion',
      fmt: ['i4:size', 'i1:type', 'i2:tag', 'i4:msize', 'S2:version'],
    ...
    102: {
      name: 'Tauth',
      fmt: ['i4:size', 'i1:type', 'i2:tag', 'S2:uname', 'S2:aname'],
    },
    104: {
      name: 'Tattach',
      fmt: [
    ...
    111: {
      name: 'Rwalk',
      fmt: ['i4:size', 'i1:type', 'i2:tag', 'i2:nqid', 'R:qids'],
    },
    112: {
      name: 'Topen',
      fmt: ['i4:size', 'i1:type', 'i2:tag', 'i4:fid', 'i1:mode'],
    },
    113: {
      name: 'Ropen',
      fmt: ['i4:size', 'i1:type', 'i2:tag', 'b13:qid', 'i4:iounit'],
    },
    114: {
      name: 'Tcreate',
      fmt: [
    ...
    115: {
      name: 'Rcreate',
      fmt: ['i4:size', 'i1:type', 'i2:tag', 'i13:qid', 'i4:iounit'],
    },
    116: {
      name: 'Tread',
      fmt: ['i4:size', 'i1:type', 'i2:tag', 'i4:fid', 'i8:offset', 'i4:count'],
    },
    117: { name: 'Rread', fmt: ['i4:size', 'i1:type', 'i2:tag', 'S4:data'] },
    118: {
      name: 'Twrite',
      fmt: ['i4:size', 'i1:type', 'i2:tag', 'i4:fid', 'i8:offset', 'S4:data'],
    },
    ...
    126: {
      name: 'Twstat',
      fmt: ['i4:size', 'i1:type', 'i2:tag', 'i4:fid', 'S2:stat'],
    },
  };
  ...
  exports.Service = {
    ...
    Tattach: function (p) {
      if (p.fid === undefined)
        return this.error9p(p.tag, 'attach requires a fid');
      if (this.fids[p.fid] !== undefined)
        return this.error9p(p.tag, 'fid already in use');
      this.fids[p.fid] = { f: this.tree, open: false };
      return this.send9p({
        type: msgtype.Rattach,
        tag: p.tag,
        qid: this.tree.qid,
      });
    },
    ...
    Twalk: function (p) {
      if (this.fids[p.fid] === undefined)
        return this.error9p(p.tag, 'fid not in use');
      if (this.fids[p.newfid] !== undefined)
        return this.error9p(p.tag, 'fid already in use');
    ...
    Twrite: function (p) {
      ...
      if (node === undefined) return this.error9p(p.tag, 'no such fid');
      if (node.open !== true) return this.error9p(p.tag, 'fid not open');
      if (node.f.qid.type & QTDIR)
        return this.error9p(p.tag, 'permission denied');
      if (node.f.write === undefined)
        return this.error9p(p.tag, 'permission denied');
  ...
  var must_deny_access = function (f, mode) {
    var reason;
    if (isDir(f)) {
      return mode & 3 ? 'directories are read only' : false;
    }
    if ((mode & 3) === 3) {
      return 'OEXEC not implemented';
    }
    if (mode & 3 && f.write === undefined) {
      return 'read only';
    }
    return false;
  };
  ...
  var read_dirent = function (service, packet, f) {
    ...
    if (packet.offset != f.bloc)
      return service.error9p(
        packet.tag,
        'seek in directory illegal ' + packet.offset + '!=' + f.bloc,
      );
  ...
  function lookup(path, getParent) {
    ...
    s = path.split('/');
    for (x in s) {
      if (getParent && x == s.length - 1) break;
      n = s[x];
      if (n === '' || n == '.') continue;
      if (n == '..') f = f.parent;
      else {
        f = f.children[n];
        if (f === undefined) throw path + ' not found';
      }
    }
    return f;
  }
